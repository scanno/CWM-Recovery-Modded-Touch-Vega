...
#define MT_X(x) (x)
//#define MT_Y(y) (y/4)		//Define max Y axis range device recognises instead of 600
#define MT_Y(y) (y)
...
static void ui_handle_mouse_input(int* curPos)
{
  ...
  if (show_menu) {
    if (curPos[0] > 0 && curPos[1] > 910) {
		//int position = gr_fb_height() - curPos[2];
		int position = curPos[2];
		//DEBUG
		//ui_print("Mouse Pressure:%d\tX:%d\tY:%d\n",mousePos[2],mousePos[0],mousePos[1]);
		//ui_print("Cur Pressure:%d\tX:%d\tY:%d\n",curPos[0],curPos[1],curPos[2]);
		//ui_print("Position:%d\n", position);
		// End DEBUG
		pthread_mutex_lock(&gUpdateMutex);
